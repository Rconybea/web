!function() {
    ...
    /* the (quadratic) function of (x) that is tangent to 
     * fx.target_fn at x0,  and matches its second derivative
     * at x0
     */
    fx.make_tangent_quadratic_fn = function(x0, fn, fn_deriv, fn_deriv2) {
	var fx0 = fn(x0);
	var dfx0 = fn_deriv(x0);
	var d2fx0 = fn_deriv2(x0);

	/* 2nd-order Taylor series expansion */
	return function(x) {
	    var dx = (x - x0);
	    return fx0 + (dx * dfx0) + (0.5 * dx*dx * d2fx0);
	}
    } /*make_tangent_quadratic_fn
	...
	for(var i=0; i<n_pt; ++i) {
		...
		//console.log("make_target_pt_v: skipping point: i=", i, "p=", p);
	    }
	}
